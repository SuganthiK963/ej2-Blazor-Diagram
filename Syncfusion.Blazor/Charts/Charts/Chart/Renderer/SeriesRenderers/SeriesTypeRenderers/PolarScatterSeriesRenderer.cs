...
{
    internal class PolarScatterSeriesRenderer : PolarRadarSeriesRenderer
    {
        ...
        private void CalculateDirection()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (point.Visible && ChartHelper.WithInRange(i - 1 > -1 ? visiblePoints[i - 1] : null, point, i + 1 < visiblePoints.Count ? visiblePoints[i + 1] : null, XAxisRenderer) && IsWithInChartArea(point))
                {
                    ...
                    if (!argsData.Cancel)
                    {
                        point.SymbolLocations.Add(ChartHelper.TransformToVisible(point.XValue, point.YValue, XAxisRenderer.Axis, YAxisRenderer.Axis, Series));
        ...
        {
            return point.XValue >= XAxisRenderer.VisibleRange.Start && point.XValue <= XAxisRenderer.VisibleRange.End && point.YValue >= YAxisRenderer.VisibleRange.Start && point.YValue <= YAxisRenderer.VisibleRange.End;
        }
