...
var PivotButton = /** @class */ (function () {
    ...
    PivotButton.prototype.setPivotButtonDrag = function () {
        for (var _i = 0, _a = [].slice.call(this.parent.parentElement.querySelectorAll('.' + (this.parent instanceof SfPivotView ? GROUPING_BAR_CLASS : PIVOT_FIELD_LIST_CLASS) + '-button')); _i < _a.length; _i++) {
            ...
            this.parent.pivotButtonModule.createDraggable(isTreeElement, this.parent instanceof SfPivotView ?
                buttonElement.querySelector('.' + PIVOT_BUTTON_CONTENT_CLASS) : buttonElement.firstElementChild, isTreeElement ? this.parent.commonActionModule.getFieldInfo(buttonElement.getAttribute('data-uid')).fieldItem : JSON.parse(buttonElement.getAttribute('data-fieldInfo')));
        }
    };
    ...
    PivotButton.prototype.nodeStateModified = function (isTreeElement, args, cancel, fieldName) {
        ...
        var element = isTreeElement || !args.element.parentElement ? undefined : JSON.stringify({ 'isvalue': args.element.parentElement.getAttribute('isvalue') });
...
var CalculatedField = /** @class */ (function () {
    ...
    CalculatedField.prototype.onCalcDragStart = function (e) {
        ...
        if (clonedNode && draggedNode && ((this.parent.options.dataType === 'pivot' && e.target &&
            e.target.classList.contains(DRAG_CLASS)) || (this.parent.options.dataType === 'olap' &&
...
var ActionBase = /** @class */ (function () {
    ...
    ActionBase.prototype.validateInputs = function (filterInfo) {
        ...
        if (sf.base.isNullOrUndefined(operand1) || operand1 === '' ||
            ...
            var focusElement = sf.base.select('#' + this.parent.element.id + '_' + type.toLowerCase() +
                (sf.base.isNullOrUndefined(operand1) || operand1 === '' ? '_input_option_1' : '_input_option_2'), this.parent.parentElement);
    ...
    ActionBase.prototype.getTreeNode = function (treeElement, top, left) {
        if (treeElement) {
            return JSON.stringify(window.sfBlazor.getDomObject('currentNode', sf.base.closest(document.elementFromPoint(left, top), 'li'))); /* eslint-disable-line */
        }
        return null;
    };
...
var Selection = /** @class */ (function () {
    ...
    Selection.prototype.mouseClickHandler = function (e) {
        ...
        if ((target.classList.contains(HEADERCELL) ||
            ...
            else if (target.classList.contains(STACKED_HEADER_CELL_DIV) ||
                target.classList.contains(CELLVALUE) || target.classList.contains(HEADER_CELL_DIV) ||
    ...
    Selection.prototype.cellClicked = function (target, e) {
        ...
        var targetElement = null;
        if (target.classList.contains(HEADERCELL) ||
            target.classList.contains(STACKED_HEADER_CELL) || target.classList.contains(ROW_CELL_CLASS)) {
            targetElement = target;
        }
        else if (target.classList.contains(STACKED_HEADER_CELL_DIV) ||
            target.classList.contains(CELLVALUE) || target.classList.contains(HEADER_CELL_DIV)) {
        ...
        if (targetElement) {
            ...
            if (!this.parent.gridSettings.allowSelection && !(target.classList.contains(EXPAND_ICON) || target.classList.contains(COLLAPSE_ICON))) { /* eslint-disable-line */
                this.parent.dotNetRef.invokeMethodAsync('CellClickedHandler', rowIndex_1, colIndex_1, e); /* eslint-disable-line */
            }
        }
    };
    ...
    VirtualScroll.prototype.update = function (mHdr, mCont, top, left, e) {
        ...
        if (this.parent.pageSettings) {
            if (this.direction === 'vertical') {
                var rowValues_1 = this.parent.options.dataType === 'pivot' ?
                ...
                if ((this.parent.scrollPosObject.vertical === section ||
                    this.parent.pageSettings.rowSize >= this.parent.scrollPageInfo.rowCount)) {
                    return;
                }
                ...
                if (this.parent.options.dataType === 'pivot') {
                    this.parent.dotNetRef.invokeMethodAsync('UpdateScrollInfo', section, this.direction).then(function (scrollPageInfo) {
                        ...
                        var pos = exactSize_1 * exactpage -
                            (_this.parent.scrollPageInfo.rowFirstLvl * rowValues_1 * _this.parent.gridSettings.rowHeight);
                        _this.parent.scrollPosObject.verticalSection = pos;
                    });
                }
            }
            else {
                var colValues_1 = this.parent.options.dataType === 'pivot' ?
                ...
                if (this.parent.options.dataType === 'pivot') {
                    this.parent.dotNetRef.invokeMethodAsync('UpdateScrollInfo', section, this.direction).then(function (scrollPageInfo) {
                        ...
                        var pos = exactSize_2 * exactPage -
                            (_this.parent.scrollPageInfo.colFirstLvl * colValues_1 * _this.parent.gridSettings.columnWidth);
    ...
    VirtualScroll.prototype.updateScrollInfo = function (element, columnCount, rowCount) {
        if (this.parent.options.enableVirtualization) {
            if (element.querySelector('.' + MOVABLECONTENT_DIV) &&
                ...
                this.virtualDiv = sf.base.createElement('div', { className: VIRTUALTRACK_DIV });
            ...
            if (element.querySelector('.' + MOVABLEHEADER_DIV) &&
                ...
                this.virtualHeaderDiv = sf.base.createElement('div', { className: VIRTUALTRACK_DIV });
            ...
            var verOffset = (mCnt.parentElement.scrollTop > this.parent.scrollerBrowserLimit) ?
            ...
            var horiOffset = (ele.scrollLeft > this.parent.scrollerBrowserLimit) ?
                (mCnt.querySelector('.' + TABLE).style.transform.split(',')[0].trim() + ',') :
                'translate(' + -(((ele.scrollLeft * this.parent.horizontalScrollScale) -
                    this.parent.scrollPosObject.horizontalSection - ele.scrollLeft)) + 'px,';
            ...
            if (this.parent && exactSize > 0 && hScrollPos <= exactSize && ele.scrollLeft > 0) {
                ...
                this.parent.scrollPosObject.horizontalSection -= (hScrollPos <= 0 ? (mCntScrollPos > hScrollPos ? mCntScrollPos : -mCntVScrollPos) :
            ...
            if (this.parent.internalGrid && (this.parent.internalGrid.options && this.parent.internalGrid.options.height !== 'auto') ||
                (this.parent.internalGrid.element && this.parent.internalGrid.element.style.height !== 'auto')) {
...
var SfPivotView = /** @class */ (function () {
    function SfPivotView(element, options, dotnetRef) {
        ...
        this.element.blazor__instance = this; /* eslint-disable-line */
    ...
    TreeRenderer.prototype.updateOlapTreeNode = function (node) {
        if (this.parent.options.dataType === 'olap' && node) {
            ...
            if (textNode.querySelector('.' + LIST_ICON) && textNode.querySelector('.' + LIST_ICON).className.indexOf(LEVEL_MEMBERS) > -1) {
                this.parent.isAdaptive ? textNode.querySelector('.' + CHECKBOX_CONTAINER).style.display = 'none' : textNode.querySelector('.' + CHECKBOX_CONTAINER).style.visibility = 'hidden'; /* eslint-disable-line */
            }
...
var SfPivotFieldList = /** @class */ (function () {
    ...
    SfPivotFieldList.prototype.getOptions = function (element, options) {
        ...
        if (this.parentElement && sf.base.select('#' + this.parentElement.id + '_title', this.parentElement)) {
            sf.base.setStyleAttribute(sf.base.select('#' + this.parentElement.id + '_title', this.parentElement), { 'width': '100%' });
        }
    };
